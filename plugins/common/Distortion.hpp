...
public:
  ...
  void process(const float *input, float *output, int framesize) override
  {
    for (int i = 0; i < framesize; i++)
    {
      ...
      value = SignalBlock::clipSample(m_clipType, value * m_drive); // clip signal
      value = m_lpf1.getSample(value);                              // filter out higher-order harmonics
      value = m_lpf1.getSample(value);                              // filter out higher-order harmonics

      // Stage 2 - diode clipper
      value = SignalBlock::clipSample(m_clipType, value * m_drive);
...
private:
  ...
  float distortionAlgorithm(float inputSample)
  {
    ...
    for (i = 0; i < 8; i++)
    {
      ...
      clipOut[i] = SignalBlock::clipSample(m_clipType, filterOut[i]);
    }
